// Infinite scrolling implementation taken from 
// https://www.youtube.com/watch?v=IFYFezylQlI&t=255s&ab_channel=HamedBahram
...
export default function InfiniteScrollCarousel(props: {
  type: 'photo' | 'video'
  initialPosts: Array<PostType> | undefined
  maxPosts: number
}) {

  const { type, initialPosts, maxPosts } = props
  ...
  async function loadMorePosts() {
    ...
    let newPosts
    if (type === 'photo') {
      newPosts = await getPhotos(nextPage)
    } else {
      newPosts = await getVideos(nextPage)
    }
    if (newPosts?.length) {
      setPage(nextPage)
